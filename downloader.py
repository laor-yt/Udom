    ...
    if site == 'youtube':
        ...
        try:
            ...
            if is_audio:
                ...
                v_stream = None
                if max_height:
                    # Find highest res stream <= max_height that is H.264 (avc1) for Telegram compatibility
                    res_streams = yt.streams.filter(type="video", file_extension="mp4").order_by('resolution').desc()
                    res_streams = [s for s in res_streams if s.video_codec and s.video_codec.startswith('avc1')]
                ...
                if not v_stream:
                    v_stream = yt.streams.filter(progressive=True, file_extension="mp4").order_by('resolution').desc().first()
                if not v_stream:
