...
def _background_yt_telegram_send(user_id_str, user_role, yt_url, format_id, num_clips, task_id=None):
    try:
        ...
        clients_to_use = []
        if target_bots:
            if "all" in target_bots:
                # Add all active custom bots belonging to user
                if user_role == 'SUPER_ADMIN':
                    import sys
                    main_app = getattr(sys.modules.get('__main__'), 'app', None)
                    if main_app: clients_to_use.append(main_app)
                for t, c in custom_bots_manager.active_custom_bots.items():
                    clients_to_use.append(c)
            else:
                for t in target_bots:
                    if t in custom_bots_manager.active_custom_bots:
                        clients_to_use.append(custom_bots_manager.active_custom_bots[t])
        
        # Fallback if no target bots specified or none found
        if not clients_to_use:
            if user_role == 'SUPER_ADMIN':
                ...
                if default_bot_token and default_bot_token in custom_bots_manager.active_custom_bots:
                    clients_to_use.append(custom_bots_manager.active_custom_bots[default_bot_token])
        ...
        async def send_clips():
            try:
                for c_idx, client in enumerate(clients_to_use):
                    for i, clip in enumerate(clips):
                        print(f"[send_telegram] Sending clip {i+1}/{len(clips)} via bot {c_idx+1}/{len(clients_to_use)}: {clip}")
                        ...
                        async def tg_progress(current, total):
                            if task_id and total > 0:
                                ...
                                clip_contribution = 50 / (len(clips) * len(clients_to_use))
                                base_percent = 50 + (c_idx * len(clips) + i) * clip_contribution
                                overall_percent = int(base_percent + (p * clip_contribution / 100))
                                TASK_PROGRESS[task_id] = {"status": "running", "percent": overall_percent, "text": f"Uploading to bot {c_idx+1}/{len(clients_to_use)}... {p}% (Clip {i+1}/{len(clips)})"}

                        await client.send_document(chat_id=int(user_id_str), document=clip, caption=caption, progress=tg_progress)
                    ...
                    try:
                        ...
                        for s in yt.streams:
                            ...
                            if is_video and is_audio:
                                ...
                                v_codec = getattr(s, "video_codec", "")
                                codec_str = f" [{v_codec}]" if v_codec and not v_codec.startswith('avc1') else ""
                                label = f"{res} {ext.upper()}{codec_str}" if res else f"{ext.upper()}{codec_str} (video+audio)"
                                key = (res, ext, 'va', v_codec)
                                type_str = 'video'
                            elif is_video and not is_audio:
                                res = getattr(s, "resolution", "")
                                v_codec = getattr(s, "video_codec", "")
                                codec_str = f" [{v_codec}]" if v_codec and not v_codec.startswith('avc1') else ""
                                label = f"{res} {ext.upper()}{codec_str} (No Audio)" if res else f"{ext.upper()}{codec_str} (No Audio)"
                                key = (res, ext, 'v', v_codec)
                                type_str = 'video'
    ...
    def do_POST(self):
        try:
            ...
            if path == '/api/yt/send_telegram':
                ...
                task_id = body.get('task_id')
                target_bots = body.get('target_bots', [])
                ...
                import threading
                threading.Thread(target=_background_yt_telegram_send, args=(str(user['user_id']), user.get('role'), yt_url, format_id, num_clips, task_id, target_bots)).start()
